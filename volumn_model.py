...
# 将模型保存为文件
joblib.dump(rf, "/data/market_maker/model/model_all_1pct_new.pkl")

# 预测2023
#X2 = df2[:-3]
#y2 = df2['all_1pct'].shift(-3)[:-3]
...
#pred_y2[(pred_y2['pred'] == 1)|(pred_y2['y'] == 1)].to_csv('/data/market_maker/model_pred_result_data/pred_all_1pct_2023_30s.csv')
...
rf1 = RandomForestClassifier(max_depth=18, n_estimators=322)
rf1.fit(X1, y1)
joblib.dump(rf1, "/data/market_maker/model/model_volumn_all_1pct_new.pkl")

X2 = df.drop(columns = ['buy_all_1pct'],axis = 1,inplace = False)[:-2]
y2 = df['buy_all_1pct'].shift(-2)[:-2]
rf2 = RandomForestClassifier(max_depth=18, n_estimators=322)
rf2.fit(X2, y2)
joblib.dump(rf2, "/data/market_maker/model/model_buy_all_1pct_new.pkl")

X3 = df.drop(columns = ['sell_all_1pct'],axis = 1,inplace = False)[:-2]
y3 = df['sell_all_1pct'].shift(-2)[:-2]
rf3 = RandomForestClassifier(max_depth=18, n_estimators=322)
rf3.fit(X3, y3)
joblib.dump(rf3, "/data/market_maker/model/model_sell_all_1pct_new.pkl")
